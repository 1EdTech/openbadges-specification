...
[[JSON-LD11-API]] defines a compaction process for JSON-LD documents, applying a context to shorten several fields of the document. The purpose of compaction is making the document to be represented in a form that is tailored to the use of the JSON-LD document directly as JSON.
...
The JSON-LD context for Open Badges [=verifiable credentials=] permits this singular values compaction in the  \`type\` attributes throught all the data model, such they can be expressed as a string – when having only one value – or an array os strings – when having multiple values.
