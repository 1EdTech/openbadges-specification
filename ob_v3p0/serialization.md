...
The JSON-LD context for Open Badges [=verifiable credentials=] permits this singular values compaction in the  \`type\` attributes throught all the data model, such they can be expressed as a string – when having only one value – or an array of strings – when having multiple values.
