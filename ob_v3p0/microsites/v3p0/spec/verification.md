...
1. Check that the OpenBadgeCredential conforms to the specification:
   ...
   - Check that the `credentialSubject` is identified by an `id` and/or an `identifier`. If neither is present, the credential does not conform to the specification.
    ...
    > **Note**: OpenBadgeCredentials created following [[VC-DATA-MODEL-1.1]] have different names for attributes used in this process. Concretely, they have `issuanceDate` and `expirationDate` instead of `validFrom` and `validUntil`, respectively.
    > The data model of these credentials and their corresponding JSON schemas,
    >  are described at [[[#verification-support-data-models]]] and
    >  [[[#json-schema-1]]], respectively.
...
1. Refresh the OpenBadgeCredential:
    ...
    > **Note**: Refresh must be completed after checking the proof so that the verifier is not spoofed into receiving a refreshed OpenBadgeCredential from a bad actor.
...
1. Optionally verify the subject (recipient):
    ...
    > **Note**: This step is optional, but RECOMMENDED when the OpenBadgeCredential has been exchanged with the verifier as one of the [[[#docformat]]].
...
1. Check that the EndorsementCredential conforms to the specification:
   ...
   > **Note**: EndorsementCredentials created following [[VC-DATA-MODEL-1.1]] have different names for attributes used in this process. Concretely, they have `issuanceDate` and `expirationDate` instead of `validFrom` and `validUntil`, respectively.
   >   The data model of these credentials and their corresponding JSON schemas,
   >   are described at [[[#verification-support-data-models]]] and
   >   [[[#json-schema-1]]], respectively.
...
1. Refresh the EndorsementCredential:
   ...
   > **Note**: Refresh must be completed after checking the proof so that the verifier is not spoofed into receiving a refreshed EndorsementCredential from a bad actor.

   > **Note**: Only perform Refresh once. That is, do not complete Refresh a second time even if the refreshed EndorsementCredential also has a `refreshService` defined.
